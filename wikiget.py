...
""" Reference:
    Code retrieved from http://www.gyford.com/phil/writing/2015/03/25/wikipedia-parsing.php
"""
class WikipediaFetcher(object):
    ...
    def wikigetcleanpage(self, page_name):

        kk = self._get_html(page_name)
        html = self._tidy_html(kk['content'])
        print html
        soup = BeautifulSoup(html,'html')
        txt = soup.get_text()
        fout = open(page_name+".txt","w")
        fout.write(txt.encode("utf-8"))
        fout.close()
        print "done"

wki = WikipediaFetcher()
##txt1 = wki.wikigetcleanpage("Sound")
txt2 = wki.wikigetcleanpage("Music")
##txt3 = wki.wikigetcleanpage("Disney")
